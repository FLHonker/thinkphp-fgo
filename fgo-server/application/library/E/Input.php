...
class E_Input {
    ...
    protected $_csrf_token_name = 'wjg_csrf_token';
    protected $_csrf_cookie_name = 'wjg_csrf_token';
    ...
    public function __construct() {
        ...
        $yafConfig = Yaf_Application::app()->getConfig();

        if (null != $yafConfig->csrf) {
            $csrfConfig = $yafConfig->csrf->toArray();
        }
        ...
        if (isset($csrfConfig)) {
            ...
            foreach (array('csrf_expire', 'csrf_token_name', 'csrf_cookie_name') as $key) {
                if (isset($csrfConfig[$key])) {
                    $this->{'_' . $key} = $csrfConfig[$key];
                }
            }
        }

        if (E_Request::getAppPrefix()) {
            $this->_csrf_cookie_name = E_Request::getAppPrefix() . $this->_csrf_cookie_name;
        }
    ...
    public function csrf_set_cookie() {
        ...
        setcookie($this->_csrf_cookie_name, $this->_csrf_hash, $expire, "", "", $secure_cookie);
    ...
    public function xss_clean($str, $is_image = FALSE) {
        ...
        $words = array(
            ...
            'applet', 'alert', 'document', 'write', 'cookie', 'window', 'htaccess', 'iframe'
        );
...
//        E_Tools::debug($str, '所有过滤后：', chr(10));
